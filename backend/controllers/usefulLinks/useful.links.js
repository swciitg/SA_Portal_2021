const usefulLinks = require("../../models/usefulLinks/useful.links");

exports.getAllLinks = async (req, res) => {
  try {
    const links = await usefulLinks.find({}).sort("priority_number");
    res.status(200).json({
      status: "success",
      data: {
        data: links,
...
exports.postLink = async (req, res) => {
  try {
    ...
    const newLink = new usefulLinks(data);
...
exports.editLink = async (req, res) => {
  try {
    ...
    const link = await usefulLinks.findByIdAndUpdate(req.params.link_id, update);
    if (!link) {
      return res
        .status(424)
        .json({ status: "Failed", message: "Invalid data" });
    ...
    const newLink = await usefulLinks.findById(link_id);
    res.status(200).json({
      status: "success",
      data: {
        data: newLink,
...
exports.deleteLink = async (req, res) => {
  try {
    const deletedLink = await usefulLinks.findByIdAndRemove(req.params.link_id);
    if (deletedLink)
      return res
        .status(200)
...
exports.getAllSublinks = async (req, res) => {
  try {
    const link_id = req.params.link_id;
    const link = await usefulLinks.findById(link_id);
    if (!link) {
      return res
        .status(424)
        .json({ status: "Failed", message: "Invalid data" });
