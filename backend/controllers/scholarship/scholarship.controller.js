...
const fs = require("fs");
const { findByIdAndDelete } = require("../../models/scholarship/scholarship.pdf");
...
exports.getScholarshipPdfData = async (req,res)=>{
    try{
        ...
        return res.status(200).json({status:"Success", results: doc.length, data: doc});
    }
    catch(err){
        console.log(err);
        return res
...
exports.getScholarshipData = async (req,res)=>{
    try{
        const EditorData = await SchlolershipEditor.find({});
        const doc = await ScholarshipPdf.find({}).sort("-creation");
        return res.status(200).json({status:"Success", Pdfs:{results: doc.length, data: EditorData}, Editor:{data: EditorData}});
    }
    catch(err){
        console.log(err);
        return res
