...
import { createsacSublink, editsacSublink } from "../../../actions/sac";
import { BASEURL } from "../../../constants";

const SublinksForm = ({ type, formData }) => {
  ...
  const formSubmitHandler = (e) => {
    ...
    if (type === "Add")
      dispatch(createsacSublink(link_id, { name, priority_number, url })).then(
        () => {
          history.push(`${BASEURL}/admin/sac/${link_id}`);
        }
      );
    else
      dispatch(
        editsacSublink(link_id, sublink_id, { name, priority_number, url })
      ).then(() => {
        history.push(`${BASEURL}/admin/sac/${link_id}`);
      });
