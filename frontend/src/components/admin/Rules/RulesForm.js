...
const RulesForm = ({ type, formData }) => {
  ...
  const formSubmitHandler = (e) => {
    ...
    if (type === "Add")
      dispatch(createRule(formData)).then(() => {
        history.push(`${BASEURL}/admin/rules`);
      });
    else
      dispatch(editRule(form_id, formData)).then(() => {
        history.push(`${BASEURL}/admin/rules`);
      });
  ...
  return (
    <>
      <h1 className="text-3xl text-black pb-6">{type} Rules</h1>

      <div className="flex flex-wrap justify-center">
        <div className="w-full lg:w-1/2 my-6 pr-0 lg:pr-2">
          <p className="text-xl pb-6 flex items-center">
            <i className="fas fa-list mr-3"></i> {type} Rules
          </p>
          <div
