...
const CoursesScreen = () => {
  ...
  return (
    <>
      <h1 className="text-3xl text-black pb-6">SA Courses</h1>

      <div className="mt-6">
        <Link
          className="px-4 py-1 text-white font-light tracking-wider bg-gray-900 rounded"
          to={`${BASEURL}/admin/courses/add`}
        >
          Add SA Courses
        </Link>
      </div>
        ...
        <div className="bg-white">
          <table className="min-w-full bg-white">
            ...
            <tbody className="text-gray-700" id="myMenu">
              {courses.map(({ name, path, link, _id }, i) => {
                return (
                  <tr key={_id}>
                    ...
                    <td className="text-center py-3 px-4">
                      <Link
                        to={{
                          pathname: `${BASEURL}/admin/courses/${_id}`,
                          formData: {
                            path,
                            link,
                            _id,
                    ...
                    <td className="text-center py-3 px-4">
                      <button
                        ...
                        onClick={() => dispatch(deleteCourse(_id))}
                      >
                        Delete
                      </button>
                    </td>
